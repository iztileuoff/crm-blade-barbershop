...
class extends Component
{
    ...
    public function fields(): array
    {
        return [
            'sms_reminder' => [
                'label' => 'Напоминание о записи (за 30 минут)',
                'hint' => 'Отправляется клиенту без Telegram перед визитом.',
                'placeholders' => ['{time}'],
            ],
            'sms_retention' => [
                'label' => 'Сообщение для удержания',
                'hint' => 'Отправляется клиенту, который давно не приходил.',
                'placeholders' => ['{days}'],
            ],
        ];
    }
...
<div class="animate-fade-in-up">
    <div class="mb-8">
        <h1 class="font-display text-4xl font-semibold uppercase tracking-tight text-content">SMS · Шаблоны</h1>
        <p class="mt-1 text-sm text-content/40">Тексты SMS-сообщений клиентам</p>
    </div>
    ...
    <div x-data="{ saved: false }"
        ...
        <div class="mb-6 flex items-start gap-2 rounded-xl border border-brass/20 bg-brass/5 px-4 py-3 text-xs text-content/60">
            ...
            <span>Eskiz отправляет только заранее одобренные тексты. После изменения шаблона согласуйте его в кабинете Eskiz, иначе SMS не доставится.</span>
        </div>
