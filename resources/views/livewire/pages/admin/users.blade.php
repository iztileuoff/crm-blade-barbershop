...
<div class="animate-fade-in-up">
    <div class="mb-8 flex flex-wrap items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-extrabold tracking-tight text-white">Пользователи</h1>
            <p class="mt-1 text-sm text-white/40">Управление доступом к системе</p>
        </div>
    ...
    @if ($showForm)
        <div class="mb-8 overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.03] shadow-xl backdrop-blur-md">
            <div class="border-b border-white/[0.06] bg-white/[0.03] px-6 py-4">
                <h3 class="text-sm font-bold text-white">{{ $editingId ? 'Изменение пользователя' : 'Новый пользователь' }}</h3>
            </div>
                ...
                <tbody class="divide-y divide-white/[0.04]">
                    @forelse ($this->users as $user)
                        <tr class="transition-colors hover:bg-white/[0.02]">
                            ...
                            <td class="px-6 py-4 text-amber-500/60">{{ App\Models\Client::formatPhone($user->phone) }}</td>
                            <td class="px-6 py-4">
                                <span @class([
                                    'inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-bold',
                                    'bg-amber-500/10 text-amber-400' => $user->isSuperAdmin(),
                                    'bg-white/10 text-white/40' => ! $user->isSuperAdmin(),
                                ])>
                                    {{ $user->role->label() }}
                                </span>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-white/20">Пользователи пока не добавлены</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
