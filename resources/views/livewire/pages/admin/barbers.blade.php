...
<div class="animate-fade-in-up">
    <div class="mb-8 flex flex-wrap items-center justify-between gap-4">
        <div>
            <h1 class="font-display text-4xl font-semibold uppercase tracking-tight text-content">Мастера</h1>
            <p class="mt-1 text-sm text-content/40">Управление командой и графиком работы</p>
        </div>
    ...
    @if ($showForm)
        <div class="mb-8 overflow-hidden rounded-2xl border border-content/[0.06] bg-content/[0.03] shadow-xl backdrop-blur-md">
            <div class="border-b border-content/[0.06] bg-content/[0.03] px-6 py-4">
                <h3 class="text-sm font-bold text-content">{{ $editingId ? 'Изменение мастера' : 'Новый мастер' }}</h3>
            </div>
                ...
                <div class="grid gap-8 lg:grid-cols-3">
                    ...
                    <div class="space-y-6 lg:col-span-2">
                        <div class="grid gap-6 sm:grid-cols-2">
                            ...
                            <div>
                                <label class="mb-1.5 block text-xs font-semibold text-content/50">Специализация</label>
                            ...
                            @if ($this->allServices->isNotEmpty())
                                <div class="sm:col-span-2">
                                    <label class="mb-2 block text-xs font-semibold text-content/50">Цены по услугам (сум)</label>
                                    ...
                                    <p class="mt-1.5 text-[11px] text-content/25">Пустое поле — используется цена по умолчанию</p>
                                </div>
                            @endif
                        ...
                        <div>
                            <label class="mb-3 block text-xs font-semibold text-content/50">График работы</label>
                    ...
                    {{-- Photo Upload --}}
                    <div>
                        <label class="mb-3 block text-xs font-semibold text-content/50">Фотография</label>
                        <div class="relative aspect-square overflow-hidden rounded-2xl border-2 border-dashed border-content/10 bg-content/[0.02] transition hover:border-brass/30">
                            ...
                            @else
                                <div class="flex h-full w-full flex-col items-center justify-center p-6 text-center">
                                    ...
                                    <p class="text-xs text-content/30">Нажмите для загрузки (PNG, JPG до 2MB)</p>
                                </div>
                            @endif
                        ...
                        <div wire:loading wire:target="photo" class="mt-2 text-xs text-brass-ink">Загрузка...</div>
                    </div>
                </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-content/[0.04]">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-content/20">Мастера пока не добавлены</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
