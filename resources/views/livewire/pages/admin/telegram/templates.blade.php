...
class extends Component
{
    ...
    public function fields(): array
    {
        return [
            'tg_new_for_barber' => [
                'label' => 'Новая запись — мастеру',
                'hint' => 'Когда клиент записался к мастеру.',
                'placeholders' => ['{time}', '{client}', '{services}', '{status}'],
            ],
            'tg_cancelled_for_barber' => [
                'label' => 'Отмена записи — мастеру',
                'hint' => 'Когда запись отменена.',
                'placeholders' => ['{time}', '{client}', '{services}', '{status}'],
            ],
            'tg_cancelled_for_client' => [
                'label' => 'Отмена записи — клиенту',
                'hint' => 'Когда запись клиента отменена.',
                'placeholders' => ['{date}', '{time}', '{barber}', '{services}', '{price}'],
            ],
            'tg_reminder_for_client' => [
                'label' => 'Напоминание — клиенту',
                'hint' => 'За 30 минут до визита.',
                'placeholders' => ['{time}', '{barber}'],
            ],
            'tg_reminder_for_barber' => [
                'label' => 'Напоминание — мастеру',
                'hint' => 'За 30 минут до записи.',
                'placeholders' => ['{time}', '{client}', '{services}', '{status}'],
            ],
        ];
    }
...
<div class="animate-fade-in-up">
    <div class="mb-8">
        <h1 class="font-display text-4xl font-semibold uppercase tracking-tight text-content">Telegram · Шаблоны</h1>
        <p class="mt-1 text-sm text-content/40">Тексты уведомлений в Telegram</p>
    </div>
    ...
    <div x-data="{ saved: false }"
        ...
        <div class="mb-6 flex items-start gap-2 rounded-xl border border-brass/20 bg-brass/5 px-4 py-3 text-xs text-content/60">
            ...
            <span>Поддерживается HTML-разметка Telegram: <code class="rounded bg-content/[0.06] px-1.5 py-0.5">&lt;b&gt;</code>, <code class="rounded bg-content/[0.06] px-1.5 py-0.5">&lt;i&gt;</code>, эмодзи и переносы строк.</span>
        </div>
                ...
                <div class="overflow-hidden rounded-2xl border border-content/[0.06] bg-content/[0.03] shadow-xl backdrop-blur-md">
                    ...
                    <div class="p-6">
                        ...
                        <div class="mt-3 flex flex-wrap items-center gap-2">
                            <span class="text-xs text-content/40">Переменные:</span>
