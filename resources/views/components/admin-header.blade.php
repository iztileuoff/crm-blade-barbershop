{{--
    16 админ-страниц не помещаются в плоский список (issue #68) — группируем в секции
    с eyebrow-заголовками. Мастер видит только «Записи»/«Заработок» (issue #74): секции
    фильтруются по ссылкам, а заголовок секции скрывается, если после фильтра она пуста.
--}}
@php($sections = [
    [
        'heading' => __('nav.section_operations'),
        'links' => [
    ...
    [
        'heading' => __('nav.section_business'),
        'links' => [
    ...
    [
        'heading' => __('nav.section_communications'),
        'links' => [
            [
                'label' => __('nav.sms'),
...
@if(auth()->user()?->isSuperAdmin())
    @php($systemSectionIndex = collect($sections)->search(fn ($section) => ($section['key'] ?? null) === 'system'))
    @php($sections[$systemSectionIndex]['links'][] = ['route' => 'admin.users', 'label' => __('nav.users'), 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z" />'])
@endif

{{-- Barbers only ever see their own appointments and their own earnings. Any section left
     without links after filtering (Business/Communications/System) is dropped below so
     no orphaned eyebrow heading renders over an empty section. --}}
@if(auth()->user()?->isBarber())
    @php($allowedRoutes = ['admin.appointments', 'admin.earnings'])
    @php($sections = collect($sections)
        ->map(fn ($section) => [
            ...$section,
            'links' => array_values(array_filter($section['links'], fn ($link) => in_array($link['route'] ?? null, $allowedRoutes, true))),
        ])
        ->all())
@endif
...
<div>
    ...
    <div class="sticky top-0 z-30 flex items-center justify-between border-b border-content/[0.06] bg-surface/80 px-4 py-3 backdrop-blur-xl lg:hidden">
        ...
        <a href="{{ auth()->user()?->isBarber() ? route('admin.appointments') : route('booking') }}" wire:navigate class="flex items-center gap-2.5">
    ...
    <aside :class="{ 'translate-x-0!': open, 'lg:w-20': collapsed }"
        ...
        {{-- Nav — grouped into sections with eyebrow headings (issue #68); a section is
             skipped entirely once role-filtering above has emptied its links. --}}
                        ...
                        <div class="space-y-1">
                            @foreach($section['links'] as $link)
                                @if(isset($link['children']))
                                    {{-- Server-side request()->routeIs() only decides the *first* paint. Inside a
                                         @persist('sidebar') region the node survives every later wire:navigate, so
                                         PHP never re-runs here again — re-derive "is a child route active" from
                                         window.location on every client-side transition instead (Livewire docs:
                                         persisted active-link highlighting needs a client-side check, not a
                                         server-rendered one). --}}
                                    @php($groupActive = collect($link['children'])->contains(fn ($child) => request()->routeIs($child['route'])))
                                    @php($childPaths = collect($link['children'])->map(fn ($child) => parse_url(route($child['route']), PHP_URL_PATH))->all())
                                    <div x-data="{
                                            expanded: @js($groupActive),
                                            groupActive: @js($groupActive),
                                            childPaths: @js($childPaths),
                                            syncActive() {
                                                this.groupActive = this.childPaths.includes(window.location.pathname);
                                                if (this.groupActive) { this.expanded = true; }
                                            },
                                         }"
        ...
        <div class="border-t border-content/[0.06] p-3 lg:hidden">
            <x-theme-toggle label="{{ __('common.theme') }}"
                            class="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-content-muted transition-colors hover:bg-content/[0.04] hover:text-content" />
        </div>
    </aside>
    @endpersist
</div>
